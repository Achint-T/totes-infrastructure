...
resource "aws_cloudwatch_log_metric_filter" "error_filter_extract" {
  ...
  pattern        = "{ $.details.cause = \"*ingestion_lambda*\" && $.type = \"ExecutionFailed\" }" # Adjust pattern to match error logs 
...
resource "aws_cloudwatch_log_metric_filter" "error_filter_transform" {
  ...
  log_group_name = "/aws/state-machine/pipeline" #update for the actual log group name
  pattern        = "{ $.details.cause = \"*transform_lambda*\" && $.type = \"ExecutionFailed\" }" # Adjust pattern to match error logs 
...
resource "aws_cloudwatch_log_metric_filter" "error_filter_load" {
  ...
  log_group_name = "/aws/state-machine/pipeline" #update for the actual log group name
  pattern        = "{ $.details.cause = \"*load_lambda*\" && $.type = \"ExecutionFailed\" }" # Adjust pattern to match error logs 
...
# Alarm for any other reason 

resource "aws_cloudwatch_log_metric_filter" "error_filter" {
  name           = "ErrorFilter"
  log_group_name = "/aws/state-machine/pipeline" #update for the actual log group name
  pattern        = "{ $.type = \"ExecutionFailed\" && $.details.cause != \"*load_lambda*\" && $.details.cause != \"*transform_lambda*\" && $.details.cause != \"*ingestion_lambda*\" }" 
